...
区间覆盖和区间加，关键在于懒标记之间的清除关系，在我们操作$ cover $标记时，要清空对应位置的$ add
 $标记，这样才能保证信息维护的正确性。具体来讲，什么叫清空对应位置的$ add $标记呢？也就是说，一旦我们更新了$ u
 $位置的$ cover $信息，我们就要立刻把$ u $位置的$ add $信息清除掉，因为显然他不会再起到作用，而且在$ pushdown $的时候我们要先处理$ cover $标记，原因仍然是我们可能会清除$ add $标记
...
我们完全可以直接在线段树上进行这个二分操作，复杂度降为$ logn $
...
#### 线段树分治
线段树分治，全称是线段树按时间分治，其思想非常简单，往往问题的模型都是，某些元素都只在一个或常数个连续的时间区间内出现，我们想到利用线段树在$ log(len) $个线段里存下这个时间区间，也就是说我们维护一颗时间线段树，其每个叶子结点都是一个确定的时刻，其他节点都代表了一段时间区间，我们把信息存进线段树里，只需要统一处理，只需要递归到每个叶子结点就可以求出每个时间对应的答案。

注意进入该节点需要加入信息，而退出该节点需要删除信息，复杂度$ O(nlogn^2) $ 
...
对于一个序列$ a $,我们设其异或差分序列为$ b $

即$ b[i]=a[i] \oplus a[i-1] $,特别的有$ b[1]=a[1] $,可以证明的是

对于$ a $序列的子区间$ [l,r] $所构成的线性基

和对于$ b $序列的子区间$ [l+1,r] $所构成的线性基 再插入$ a[l] $

这两个线性基完全等价 因此我们把区间异或转化为单点异或

由于线性基合并的复杂度为$ log^2(v) $，因此总复杂度为$ nlognlog^{2}(v) $
...
对于不新开节点的做法，我们往往离线询问到对应的节点上，然后在递归合并$ dfs $时，当$ u $节点信息合并完成，我们顺势处理$ u
 $节点的所有询问，但是之后该节点的信息会被覆盖而不能再使用
...
#### 维护树链
树链剖分得到$ dfn
 $序列和重链信息，可以把链转成$ logn $个区间，于是我们可以用线段树维护树上链的信息
...
#### 吉司机线段树
$ SegmrntTree \ Beats
 $，又名吉司机线段树，是专门用来解决一类区间取$ min
 $，区间取$ max
 $问题，一般来说这类问题只需要普通的懒标记线段树就能解决，但是一旦加上区间加操作，这个问题变得十分困难，这和最大子段和问题类似。$ Beats $就是解决这一类问题的利器。

我们以区间取$ min $为例子,我们记录区间$ max
 $,区间次大值$ sema $,最大值出现次数$ cma $,那么我们进行区间$ min $操作时,我们设当前取$ min $的值为$ val $,如果$ max<=val $，显然我们不需要操作，如果$ sema<=val  $并且$ max>val $，那么我们维护当前的$ max
 $和$ sum $，即$ max=val,sum-=cma*(max-val) $然后结束，否则我们继续递归下去。

不难发现问题所在，我们没有递归所有节点更改，我们也没有打懒标记，这就是$ Beats $的独特之处

实际上我们的$ max $值本身就是一个懒标记可以使用，举例来说，在我们$ pushdown
 $的时候，如果$ tr[u].max<tr[lc].max $，这个式子本身就存在矛盾，因此我们知道是我们的操作导致的，我们在此进行修正从而维护了节点信息正确,对于具体的数据变化我们要具体分析,$ pushdown $函数需要随机应变

根据论文的证明，时间复杂度$ O(nlognlogn) $
...
当然$ Beats $对于数字的修正效果不止于取$ min $或取$ max $，我们可以做把区间$ [l,r] $内$ >=p $的数变成$ q(q>p) $类似的操作。
...
#### 猫树
猫叔是一类不支持修改但支持快速查询的线段树，具体的说，对于一个线段树节点$ Node(l,r) $，我们考虑从$ mid $分开，对$ [l,mid] $维护后缀和，对$ [mid+1,r] $维护前缀和，不难发现我们一定可以拼出跨越$ mid $的任意一个区间的信息，只需要合并对应的前缀信息和后缀信息。那么对于一个询问$ [l,r] $，我们不妨设节点$ Node(l,l) $编号为$ L $,$ Node(r,r) $编号为$ R $，我们考虑线段树节点$ LCA(L,R) $，不难发现该节点一定包含$ [l,r] $且其$ mid $一定在$ [l,r] $范围内。那么我们只要在$ LCA(L,R) $得到查询$ (l,r) $的信息即可。

那么询问复杂度为$ O(单次合并+求LCA) $。

为了方便求$ LCA $，我们不妨把数组长度补位$ 2^k $，这样建出的树会是一颗完美的满二叉树，那么我们对于线段树的每一层维护一个$ info $数组即可，对于每次查询，我们从$ L,R $跳到它们对应$ LCA $所在的层数，公式为$ log[L]-log[L \oplus R]-1 $，我们在该层统计我们想要的信息即可，这样求$ LCA $的复杂度为$ O(1) $
...
#### 维护前后缀最大/最小值和
你需要维护一个排列（序列也行）,有单点修改操作，存在一个函数$ f(x) $,每次操作后，需要你输出所有位置$ i $的函数值之和,其中$ i $位置的值必须是前缀最小值,我们记这个和为$ val $

我们考虑用线段树维护，每个区间维护$ mi $和$ val $,问题在于修改时如何合并两个区间的$ val $，我们分两部分考虑

注意到前缀最小值的性质，一个区间的左子区间一定有贡献，因为第一个一定满足，然后我们根据左子区间的最小值$ tr[lc].mi $，记这个值为$ k $,来递归求解右子区间的贡献,我们写一个$ count $函数

在递归函数$ count $中,如果$ tr[lc].mi>k $那么说明左区间一定没贡献，我们直接递归右边，否则，左区间一定存在贡献，当左区间存在贡献时，右区间的贡献一定全部有效，因为右区间能贡献的值一定小于$ tr[lc].mi $,因此我们单次操作复杂度$ O(lognlogn) $,实际上是把$ O(1)
 $的$ pushup $变成了$ O(logn) $
...
##### 矩形面积并
需要注意的是离散化以后每个点代表的是一段距离$ [l,r) $，其中$ r $表示下一个点的左端点。
...
定义$ (a,b,c)->x $表示$ x:=min(c,max(b,x+a)) $

考虑合并标记即

$ (a,b,c)=merge((a1,b1,c1),(a2,b2,c2)) $

等价于$ (a2,b2,c2)->((a1,b1,c1)->x) $

结论是

$ a=a1+a2 $

$ b=max(b2,min(c1+a2,b1+a2)) $

$ c=min(c2,max(c1+a2),b2) $
...
主要面向的问题是，对节点$ u
 $，询问其子树内深度小于等于$ d $的信息，注意到虽然完整子树的$ dfn $序连续，但是只截取部分深度是不连续的。但按我们这样建立的主席树，相同深度的节点将是同一个版本，也就可以解决这一类问题。但是其他子树内也存在深度小于等于$ d $的节点，怎么办呢？注意到主席树实际上可以限制两维信息，分别是版本区间和内层值域区间，我们只需要限制值域为$ [dfn[u],dfn[u]+siz[u]-1] $即可
...
考虑使用安全的$ gp\_hash\_table $来代替（极快）
...
安全的$ umap $(比$ gp\_hash\_table $慢很多)
...
## 平衡树
平衡树是一种$ BST $，也就是二叉搜索树。在理想状态下我们的操作复杂度为$ O(logn) $,但是在退化为链为复杂度将退化为$ O(n) $，因此我们往往通过某些手段来尽可能维护树的平衡性，因此衍生出了不同种类的平衡树
...
### FHQtreap
$ FHQ
 $维护平衡的方式是它对每个节点都随机一个类似于优先级的权值来维护平衡，通过这种操作使得树高的期望值为$ O(logn) $,在代码中我们用$ key $表示，其核心操作是$ Split $和$ Merge $，具体的说,$ Split
 $函数分别有四个参数，表示把根节点$ U
 $分成$ x $和$ y $两棵子树，其左子树的值小于等于$ val $，右子树的值大于$ val $，

$ Split
 $最最最容易出错的地方是我们每次$ Split
 $后必须重新合并！，不合并将会破坏整棵树的结构！

我们用root表示根，而$ merge $函数的返回值为$ int $,也就是说当我们$ split(root,val,x,y) $后，需要进行操作$ root=merge(x,y) $。这两个函数十分容易理解，$ split

 $根据$ BST
 $左小右大的性质递归分裂即可，对于$ merge $，我们需要用到一开始随机的优先级权值来决定合并顺序，以此维护树高的期望为$ logn $

#### 维护值域
我们可以理解为是进阶版的超级$ set $和$ multiset $,几乎支持维护数组值域上所有的操作，且均为单次操作$ O(logn) $
...
### Splay
$ Splay $是$ LCT $的基础，只有基于$ Splay $的$ LCT $复杂度才是优秀的$ nlogn $

$ Splay
 $有两个核心操作，$ rotate $和$ splay $,其中$ rotate $分为左旋和右旋，相当于是在保证中序遍历不变的情况下进行旋转，有了$ rotate $之后我们就可以进行核心操作$ splay
 $，$ splay $的作用就是根据当前树不同的形态，采用不同的旋转策略，尽可能减小树高来优化复杂度，也就是通过$ splay $函数维持树的平衡,多$ Splay $可以保证复杂度。
...
### 动态树
动态树(Link cut Tree)，简单理解就是把平衡树建在树上，由于平衡树的特性使动态树有了动态连边动态删边的能力，但实际上即使不需要动态操作树的结构,我们也可以用$ LCT $求解一些问题，比如树链最大子段和问题，用$ LCT $来做显得十分自然,用线段树做则显得十分复杂



$ LCT $把一整颗树分为分为多条按深度递增的链，分法并不唯一，然后我们对每条链以深度为标准都开一颗$ Splay $维护。然后我们引出实链剖分的概念，我们定义在$ Splay $中的边都是实边，连接不同$ Splay $的边为虚边，那么不难发现，一个点的多个儿子不能同时出现在一个$ Splay $中，所以每个点只有一个实儿子，其余全是虚儿子，对于虚儿子所在的$ Splay $，该$ Splay $所在的根通过虚边连向该点。换句话说，$ Splay $中的根节点的父节点是该$ Splay $中深度最小节点的父亲，且满足认父不认子的特征。

然后我们就可以讲操作函数了

$ access(x) $表示把$ x $和原树根节点之间（深度最小点）的路径打通，即路径上全是实边，且$ x
 $点变成该$ Splay
 $深度最大点，我们先把$ x $转到所在$ Splay $的根，此时的右儿子深度一定大于我，因此直接置空，置空的意义就是实边转为虚边，我成为了深度最大点。然后我跳到当前节点的父节点$ fa[x] $，来到了上一个$ Splay $深度最大的节点,因为要打通路径，我们继续把$ fa[x] $转到根，即$ splay(fa[x]) $，并且把现在的右儿子设为$ x $,即$ fa[x].rson=root $，意思就是把这两个$ Splay $之间的路径打通了，不断递归下去直至父节点不再存在，就说明我们已经打通了所有的路径,注意在此过程中需要不断$ pushup $更新信息

$ makeroot(x) $表示把$ x $变成原树的根，因为有时候我们需要的路径有可能深度并不递增，我们无法将他们放置在同一个$ Splay $中，因此我们需要换根操作。我们知道$ access(x) $以后$ x $是所在$ Splay $深度最大点，那么我们$ Splay(x) $后$ x $将不存在右儿子,那么此时我们对整个$ Splay $打翻转标记，那么翻转后的$ x $将没有左儿子，成为了原树的根,当然也需要$ pushup $

$ findroot(x) $表示找到$ x $在原树中的根，我们仍然先$ access(x),splay(x) $,然后不断跳左儿子，并且要$ pushdown $，直至找不到做儿子为止，也就找到了根。最后要$ Splay() $保证复杂度

$ split(x,y) $表示分离出$ x-y $的链并将信息整合到了$ y $，怎么做呢，很显然我们先$ makeroot(x) $，然后$ access(y) $，再$ Splay(y) $即可

$ link(x,y) $,即把$ (x,y) $之间连一条边，直接把$ x $变为原树的根，令$ tr[x].fa=y $即可

$ cut(x,y) $,即把$ (x,y) $之间的边断开，我们直接$ split(x,y) $,然后使$ tr[x].fa=tr[y].lson=0 $即可

当然在代码中我们判断了非法连边断边的操作，但一般题目都保证数据合法

#### 维护树链信息
有了以上的这些函数，我们已经能动态维护森林，并且维护链上的信息了。$ LCT $维护链信息最经典的应用是动态$ MST $，因为我们能轻易求出链上边权最小值，并且动态加边断边,对于懒标记的使用，几乎和平衡树没有区别。
...
#### 维护子树信息
根据上文，我们发现$ LCT $似乎并不能维护子树信息，事实上这的确是$ LCT
 $的弱项，但我们仍有办法维护。

我们发现对于点$ x $,我们只能维护出它的实儿子的信息，实际上是从$ x $出发的一条链。那么我们不难想到，只要能够维护虚儿子信息的总和，我们把他们合并起来便是子树信息了。

对于每次询问，我们$ split(root,u) $，此时$ u $已经被转到根上，那么$ u $的子树信息和为$ u.exInfo+u.val $
...
#### 维护边双连通分量
$ LCT $同时能动态维护边双连通分量来维护图的连通性，怎么做呢？我们维护两个并查集，其一为全局连通性并查集，其二为$ LCT $用并查集，在加边$ (a,b) $时，我们用全局并查集判断这两点是否联通，假如不联通，我们$ link(a,b) $,否则我们$ split(a,b) $，并通过$ dfs $把信息整合到$ b $节点上，然后我们将$ b $的左儿子和右儿子置空，代表我们把该边双连通分量缩点，在$ dfs $过程中我们还得维护内层并查集，也就是把经过的节点的父亲置为$ b
 $，代表他们缩点以后变成了$ b
 $点。

总的来说，外层并查集维护全局图的连通性，内层并查集维护$ LCT $内所有点目前真正的祖先
...
#### 维护最大子段和
单独拿出来，实际上是因为最大子段和的维护十分特殊，涉及到$ lmax $和$ rmax $,由于$ LCT $大量用到$ reverse $操作，我们需要在$ pushdown $交换左右儿子时同时交换$ lmax $和$ rmax $，这一点十分关键且容易遗漏
...
先说按秩合并，按秩合并的本质是维护树高，也就是说按某个标准合并可以保证树高不会太高，假如我们用$ Height $数组表示树高，那么合并时如果$ Height(x)<Height(y) $，我们就把$ x $合并给$ y
 $。当然我们的标准不止有树高，也可以根据子树大小按秩合并

再说路径压缩，路径压缩实际上是在$ root
 $过程中调整并查集结构的做法，实际上也是在减小树高，如果$ y $是$ x $所在连通块的根，且是$ x $的$ k $级祖先，那么我们第一次查找$ root(x) $自然需要跳$ k
 $次，但我们这次操作之后，自然知道$ pre(x)=y $,于是我们在$ root $过程中重新调整$ pre(x) $,使得树高直接减小为$ 1 $
...
我们以$ val $为例，$ val(u) $代表$ u
 $到其根节点的边权异或和。

在带权并查集中我们仍然能使用路径压缩，但需要在压缩过程中维护正确的信息,也就是说，在我更改我的$ pre
 $时，还要维护$ val $的正确性，因为我的初始路径在压缩后，这个路径的信息也应该被压缩到我上面

举例来说,一开始路径为$ u\implies pre[u] \implies root $,我们路径压缩后变为$ u \implies root $，但是我们的$ u $节点丢失了$ pre[u]\implies root $路径上的信息，我们在递归时直接把信息整合到到$ val[u] $上即可,除了$ root $的直接儿子外，其他儿子都需要维护$ val $。
...
## 根号算法
### 分块
我们把一个序列分成若干个块，我们记块长为$ B $，序列长度为$ N
 $,那么共有$ N/B $个块，我们考虑对一个区间$ [l,r] $操作，我们考虑线段树的懒标记思想，我们对完整的块打标记，对不完整的块暴力操作。那么复杂度为块数+块长，即$ N/B+B $，当$ B=sqrt(N) $时最小。
...
第二是维护分块平衡，在分块中我们可能面临插入或者删除操作，当这样的操作过多时块的大小不再平衡，最坏情况下块会变成$ B+q $，我们考虑每$ q
 $次修改就重新构建整个分块

### 莫队
莫队是针对形如$ Q $次询问，每次给定$ L,R $询问区间信息的暴力算法，其思想一定程度上基于分块，通过把序列分块的方式，我们离线所有询问，询问区间$ [L,R] $按左端点所在块编号为第一关键字，右端点为第二关键字排序，然后操控指针移动暴力求解。可以证明在这样的操作顺序下，指针移动次数是$ n\sqrt n
 $数量级的,所以复杂度也为$ O(n\sqrt n) $ 
...
#### 带修莫队
我们考虑给询问加上一维时间轴信息，该时间对应着该询问之前最后一次修改操作的编号，我们加入第三个修改指针，每次和$ l,r $一样移动修正贡献即可。我们取$ B=N^{2/3} $，复杂度为$ O(N^{5/3}) $。
...
#### 树上莫队
我们通过树的欧拉序（括号序）把链转化为序列来做，我们不妨为$ in $，$ out $表示欧拉序，那么我们需要对询问分情况讨论：若询问$ (x,y) $是一条链（即$ lca=x $或$ lca=y $），我们查询的区间为$ [in(x),in(y)] $，这样做的原理是非链上的节点会出现两次，我们第二次遍历删除其贡献即可。若$ (x,y) $不是一条链，我们查询的区间为$ [out(x),in(y)] $,但这样会漏掉$ LCA $的贡献，我们需要单独统计。由于欧拉序，空间需要开两倍，请格外注意。
...
#### 莫队二次离线
用于解决莫队指针移动复杂度不正确的问题，也就是说我们不能$ O(1) $移动指针，我们可以考虑存下一些不能快速计算的贡献，在莫队执行完后单独统计。

我们以区间询问逆序对为例，我们考虑右扩展$ [l,r]->[l,r+1] $时新增的贡献是$ [l,r] $中大于$ a[r+1] $的数字个数，我们用$ f(x,y) $表示$ [1,x] $中大于$ a[y] $的个数，那么该贡献可以写$ f[r,r+1]-f[l-1,r+1] $

前一部分我们可以预处理出，后一部分我们考虑存下该查询，最后统一查询即可。那么我们统一查询的数量共有$ nsqrt(n) $个，问题形如求$ [1,x] $的前缀中$ y $的排名。这个我们可以考虑用$ O(sqrt(n)) $的插入和$ O(1) $查询的值域分块解决。

（值域分块$ details $:我们插入一个数$ x $时，暴力更新$ x $所在块的每个位置的答案，对其他块打懒标记即可，复杂度为$ O(sqrt(n)) $,对于查询，就是每个点的答案加上所在块的懒标记答案）

需要注意的是我们解决的是答案的变化量，也就是最后前缀和才是正确的答案

## 离线分治算法
#### CDQ分治
CDQ分治是一种高效统计点对数量的分治算法，具体的说，我们对于一个区间$ [l, r] $统计满足某种条件的点对数量，我们考虑跨过中点$ mid $的贡献，对于没跨中间的贡献我们可以递归两边统计。

我们以三维$ (a,b,c) $偏序为例，我们首先在最外层对$ a $排序,然后我们单独对$ [l,mid] $的$ b $排序，也对$ [mid+1,r] $的$ b $排序，这样我们可以用双指针扫描左边和右边区间使得$ b $满足条件，利用一个树状数组维护$ c $即可。复杂度为$ O(nlognlogn) $

#### 整体二分
把所有的修改和询问视作一个序列按顺序存起来，我们用一个整体的二分分治函数去求解所有问题的答案，用$ [l,r] $表示当前处理的操作区间，$ [L,R] $表示当前区间答案的值域范围，递归出口是$ l>r $或者$ L=R $,当$ L=R $时区间$ [l,r] $的答案均为$ L $。
...
假如说原本的数组通过异或得到的所有数字的集合为$ S $，那么$ Base $也可以得到$ S $同时$ Base $的大小最小，其大小上界为$ log(V) $

通过导出的$ Base $我们可以贪心解决异或最大值，$ k $大值等问题。
...
#### 前缀线性基
可以理解为可持久化线性基，本质就是建立了不同版本的线性基。我们用$ p[idx][i] $表示第$ idx $个版本的线性基第$ i $个位的基，用$ pos[idx][i] $表示该基最晚出现的位置（贪心），如果新加入的数可以导出某个基且出现位置更晚，我们就和原本的基贪心的交换位置。因为我们对于区间询问$ [l,r] $，我们会查$ r $版本的线性基，如果某个位置$ i $的基的最晚出现位置$ >=l $，就说明这个基是可用的。
...
前缀线性基也是可以建在树上的，类似于主席树维护树上信息，我们每次基于父节点的版本插入新版本。但查询链上的线性基不能差分，我们考虑对于一条链$ (u,v) $，我们求出$ (u,lca) $的线性基和$ (v,lca) $的线性基暴力合并即可，这样复杂度为$ O(logvlogv) $
...
### 可并堆（$ pbds $实现）
请注意大根堆是$ std::less $，小根堆是$ std::greater $

使用教程：

我们在$ solve $函数内自己开一个迭代器数组，当加上某个数，删除某个数，修改某个数，需要根据返回的$ it $自动更改迭代器为新迭代器或者$ nullptr $，如果是删除堆顶，我们一开始传入数据时传入每个数字的下标，删除堆顶时手动把堆顶的下标对应的迭代器置为$ nullptr $
...
#### 笛卡尔树
笛卡尔树是把一个序列建立成一颗二叉搜索树，整棵树的中序遍为长度为$ n $的顺序排列（即$ (1,2,3,4....n) $,同时满足堆的性质，以小根堆为例，也就是$ val[x]<=val[lc[x]],val[x]<=val[rc[x]] $
...
显然该过程可以保证笛卡尔树的两个性质，由于每个点只会入栈一次和出栈一次，复杂度为$ O(n) $
...
#### set维护不交区间
如果认为区间$ [1,4] $和区间$ [5,8] $不相邻的话 我们把所有坐标乘$ 2 $最后除$ 2 $即可。
